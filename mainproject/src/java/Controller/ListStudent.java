...
public class ListStudent extends HttpServlet {
               ...
               int tpage=0;
                    for(int i=0;i<l.size();i++){
                        if(l.get(i).getName().toLowerCase().contains(text.trim().toLowerCase())){
                            if(!li.contains((i+1)%3==0?(i/3):((i/3)+1))){
                            li.add((i+1)%3==0?((i+1)/3):(((i+1)/3)+1));
                            }
                        }
                    }   
                    if(li.size()>0){
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if(request.getParameter("search")!=null){
            ...
            if(!request.getParameter("content").trim().equals("")&&request.getSession().getAttribute("id")!=null){
                c.sendcomment(request.getParameter("content"), (String)request.getSession().getAttribute("id"),request.getParameter("comment"), new SimpleDateFormat("yyyy-MM-dd").format(new Date()));
