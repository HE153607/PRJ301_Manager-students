...
public class Parent extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        if (!er.equals("")) {
            ...
            Student s = c.StudentbyId((String) request.getSession().getAttribute("parent"));
                ...
                ArrayList<Student> attends = c.tableattend(s.getId());
                if (attends.size() > 0) {
                    request.setAttribute("attend", attends);
                }
                ArrayList<Student> comments = c.getcomment(s.getId());
                if (comments.size() > 0) {
                    request.setAttribute("comment", comments);
                }
                request.getRequestDispatcher("phuhuynh.jsp").forward(request, response);
        }
        
        else request.getRequestDispatcher("phuhuynh.jsp").forward(request, response);
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (request.getParameter("id") != null && !request.getParameter("id").trim().equals("")) {
            ...
            if (p != null && b) {
                if (request.getParameter("me") != null && request.getParameter("me").equals("me")) {
